...
We've already wrangled out data - but let's look at the data frame to make sure it is still correct. A quick way to look at the dataframe is with the [`skimr` package](https://cran.r-project.org/web/packages/skimr/vignettes/skimr.html).
...
First, install and load the package. Normally you would do this above but we want to make sure you know which packages aer used with the new functions.
...
Or, skim by a group such as the `subject` variable
...
You can read more about ggplot in the book ["GGPLOT: Elegant Graphics for Data Analysis"](https://ggplot2-book.org/introduction.html). You can also find lots of inspiration in the [r-graph gallery](https://r-graph-gallery.com/) that includes code.
...
#### Level b. Add another layer with labs
...
-   geom_histogram() \* this is already there
...
-   `geom_point()` function - Scatter plot
...
#### Level b. Add **Scale** with a different color.
...
#### Level c. Add another layer with labs
...
Plot the linear model with: 
`geom_smooth` function and method = "lm" which fits a linear model, giving the line of best fit.
